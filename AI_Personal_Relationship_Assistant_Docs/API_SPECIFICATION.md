...
- `/auth` — register, login, me, logout, fcm-token, `PUT /auth/password` (change password: `current_password`, `new_password`, `new_password_confirmation` — requires the current password, revokes every other active token on success)
