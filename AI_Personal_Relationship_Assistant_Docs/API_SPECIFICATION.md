...
- `/contacts/{contact}/messages/import` — upload a WhatsApp "Export Chat" .txt file (multipart: `file`, `owner_name`) to backfill historical messages for a contact, then immediately re-runs memory extraction + style profile analysis on the imported history. This is how the AI learns the *owner's* actual voice — normal webhook traffic never captures what the owner types from their own phone, only inbound messages.
