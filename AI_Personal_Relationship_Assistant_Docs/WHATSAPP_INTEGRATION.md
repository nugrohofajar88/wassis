...
**Auto-reply toggle is two-tier** (pattern borrowed from the sibling `chatbot-crm` project): a reply is only auto-sent when **both** are true — the owner's global `auto_reply_enabled` Setting, AND the specific contact's `ai_enabled` column (`contacts` table, default `true`). This lets you opt in to auto-reply generally while still forcing manual replies for specific contacts (e.g. close family) via `PUT /api/contacts/{id}` with `{"ai_enabled": false}`.
